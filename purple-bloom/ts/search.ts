(function (): void {
  ...
  const grid = document.querySelector<HTMLElement>(".catalog__grid");
  const empty = document.querySelector<HTMLElement>(".catalog__empty");
  const filterButtons = Array.from(document.querySelectorAll<HTMLElement>(".filter-list__link"));

  if (!form || !input || !grid) return;

  const gridEl = grid;
  const inputEl = input;
  const cards = Array.from(gridEl.querySelectorAll<HTMLElement>(".product-card"));
  ...
  function getCategoryFromUrl(): string {
    ...
    const match = filterButtons.find(
      (btn) => (btn.getAttribute("data-category") ?? "").toLowerCase() === raw.toLowerCase()
    );
