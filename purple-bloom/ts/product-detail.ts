(function (): void {
  ...
  function mediaHtml(p: PurpleBloomProduct): string {
    if (p.imageSrc) {
  ...
  const addButton = root.querySelector<HTMLElement>('[data-action="add-to-cart"]');
  ...
  const favoriteButton = root.querySelector<HTMLElement>('[data-action="add-to-favorite"]');
