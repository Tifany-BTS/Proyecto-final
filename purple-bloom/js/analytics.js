"use strict";
function pbTrack(event, properties, callback) {
    if (window.mixpanel) {
...
function pbTrackBeforeNavigate(event, properties) {
    pbTrack(event, properties);
    const mp = window.mixpanel;
    if (!mp || typeof mp.get_distinct_id !== "function" || typeof navigator.sendBeacon !== "function")
        return;
